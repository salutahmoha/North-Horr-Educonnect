...
import { WriteContainer, LabelStyled, InputStyled, ButtonStyled } from '../../components/StyledComponents/WriteStyled';
...
function Write() {
    ...
    //     onSuccess: (data) => {
    //         if (data?.id) {
    //             toast("Report Created Successfully", { theme: "toast-success", duration: 3000 });
    //             navigate(`/reports/${data.id}`); // Redirect to the report's full view
    //         } else {
    //             toast("Failed to retrieve the report ID", { theme: "toast-error", duration: 3000 });
    //         }
    //     },
    //     onError: (error) => {
    ...
    //         toast("Please fill in all fields", { theme: "toast-error", duration: 3000 });
    //         return;
    //     }

    //     if (role !== 'admin') {
    //         toast("You must be an admin to create a report.", { theme: "toast-error", duration: 3000 });
    //         return;
    //     }
    ...
    return (
        <div>
            <EduNavbar />
            <WriteContainer>
                <LabelStyled htmlFor='schoolname'>Name of the School</LabelStyled><br />
                <InputStyled 
                    type="text" 
                    id='schoolname' 
                ...
                <LabelStyled htmlFor="image">Upload Image</LabelStyled><br />
                <InputStyled 
                    type="file" 
                    id='image' 
                    placeholder="Upload Image"  
                    style={{ width: '100%', height: '2rem', outline: 'none', margin: '.5rem 0', padding: '.3rem', cursor: 'pointer' }}
